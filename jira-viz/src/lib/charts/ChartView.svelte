<script lang="ts">
  ...
  const issues = $derived(
    chartStore.issues.length ? chartStore.issues : fallbackIssues,
  );
  ...
  $effect((): void => {
    if (dateFields.length) {
      if (!dateField || !dateFields.includes(dateField)) {
        dateField =
          dateFields.find((f) => f === "created" || f === "createdDate") ??
          dateFields[0];
      }
    } else {
      dateField = "";
  ...
  function triggerAggregate(): void {
    ...
    _aggTimer = setTimeout(async () => {
      ...
      console.log("[CV] triggerAggregate →", {
        axisX,
        axisY,
        axisType,
        isCategoricalY,
        issueCount: dateFilteredIssues.length,
      });
  ...
  const aggregatedOption = $derived.by((): EChartsOption | null => {
    ...
    const hasData =
      agg.pie_data?.length || agg.scatter_data?.length || agg.series?.length;
    console.log(
      "[CV] aggregatedOption: title=",
      agg.title,
      "| hasData=",
      !!hasData,
      "| pie=",
      agg.pie_data?.length ?? 0,
      "| scatter=",
      agg.scatter_data?.length ?? 0,
      "| series=",
      agg.series?.length ?? 0,
      "| axisType=",
      axisType,
    ...
    if (!hasData) {
      console.warn(
        "[CV] aggregatedOption: agg returned but hasData=false → chart will not update",
      );
      return null;
    }
    ...
    if (agg.pie_data?.length) {
      console.log(
        "[CV] aggregatedOption → pie (",
        agg.pie_data.length,
        "slices)",
      );
      const entries: [string, number][] = agg.pie_data.map((p) => [
        p.name,
        p.value,
      ]);
    ...
    if (agg.scatter_data?.length) {
      ...
      return {
        ...
        series: groups.map((g, i) => ({
          ...
          name: g || "Items",
          data: agg
            .scatter_data!.filter((p) => p.group === g)
            .map((p) => [p.x, p.y]),
    ...
    if (axisType === "scatter" && agg.series?.length) {
      console.log(
        "[CV] aggregatedOption → scatter (bar fallback, series count=",
        agg.series.length,
        ")",
      );
      return {
        ...
        xAxis: {
          ...
          axisLabel: {
            color: "#94a3b8",
            fontSize: 10,
            rotate: agg.x_axis.length > 6 ? 30 : 0,
            interval: 0,
          },
    ...
    console.log(
      "[CV] aggregatedOption → bar/line series (count=",
      agg.series.length,
      ", x_axis=",
      agg.x_axis.length,
      "categories)",
        ...
        return {
          ...
          ...(s.chart_type === "bar"
            ? {
                itemStyle: {
                  color: sem
                    ? semanticBarGradient(sem)
                    : paletteGradient(stableColorIndex(s.name)),
                  borderRadius: s.stack
                    ? ([0, 0, 0, 0] as const)
                    : ([12, 12, 0, 0] as const),
                },
                barMaxWidth: 40,
                ...
                backgroundStyle: {
                  ...
                  borderRadius: [12, 12, 0, 0] as const,
                },
                label: s.stack
                  ? {
                      show: true,
            ...
            : {
                ...
                showSymbol: false,
                lineStyle: {
                  color: sem ?? paletteColor(stableColorIndex(s.name)),
                  width: 2,
                },
                label: {
                  show: agg.x_axis.length <= 15,
                  position: "top" as const,
                  color: "#e2e8f0",
                  fontSize: 9,
                  fontFamily: "Inter, system-ui, sans-serif",
  ...
  function fmtJiraValue(val: unknown): string {
    ...
    if (typeof val === "string") {
      ...
      if (
        val.includes("com.atlassian.greenhopper") ||
        val.includes("Sprint@")
      ) {
  ...
  function isDateLike(key: string, sample: unknown): boolean {
    return (
      ...
      JiraDateFormatter.isIsoDate(String(sample ?? ""))
    );
  }
  ...
  const filterableMap = $derived.by((): Record<string, FilterInfo> => {
    ...
    for (const key of displayFields) {
      const sample = resolveIssueField(
        issues.find((i) => resolveIssueField(i, key) != null) ?? {},
        key,
      );
        ...
        (() => {
          ...
          for (const issue of issues) {
            ...
            if (v == null || v === "") label = "Empty";
              ...
              label = v.length ? fmtCell(key, v) : "Empty";
  ...
  const filteredIssues = $derived.by((): ApiIssue[] => {
    ...
    return dateFilteredIssues.filter((issue) =>
      active.every(([key, vals]) => {
        ...
        const raw =
          resolveIssueField(issue, key) ??
          (rk ? resolveIssueField(issue, rk) : undefined);
        let val: string;
        if (raw == null || raw === "") val = "Empty";
        else if (Array.isArray(raw))
          val = (raw as unknown[]).length ? fmtCell(key, raw) : "Empty";
  ...
  const LINE_INTENT_RE =
    /\b(multi.?line|line\s+chart|multiline|trend\s+by|over\s+time\s+by)\b/i;

  const specs = $derived.by((): Record<string, SpecEntry> => {
    // Auto charts always built from the full (or filtered) issue set
    const src =
      reactToFilters && hasActiveFilters ? filteredIssues : dateFilteredIssues;
    const auto = buildAllSpecs(
      src,
    ...
    if (aggregatedOption) {
      const label =
        chartStore.aggregated?.title ?? chartStore.chartSpec?.title ?? "Chart";
      const result = {
        explicit: { label, icon: axisType, option: aggregatedOption },
        ...auto,
      };
      console.log(
        "[CV] specs → explicit+auto, tabs:",
        Object.keys(result),
        "| activeTab:",
        activeTab,
      );
    ...
    if (chartStore.chartSpec) {
      ...
      const opt = fromExplicitSpec(
        chartStore.chartSpec,
        issues,
        features.charts.animation,
      );
      if (opt)
        return {
          explicit: {
            label: chartStore.chartSpec.title || "Chart",
            icon: chartStore.chartSpec.type || "bar",
            option: opt,
          },
          ...auto,
        };
      ...
      if (groupField) {
        const xField =
          autoDetectGroupField(issues, [groupField]) ??
          Object.keys(issues[0] ?? {}).find((k) => k !== groupField) ??
          "";
        console.warn(
          "[ChartView] chart_spec null, line intent — xField:",
          xField,
          "groupField:",
          groupField,
        );
        const opt = buildGroupedCategorical(
          issues,
          xField,
          groupField,
          "count",
          "line",
          `Issues by ${xField} / ${groupField}`,
          features.charts.animation,
        );
  ...
  $effect(() => {
    ...
    if (!activeTab || !tabKeys.includes(activeTab)) {
      const preferred = tabKeys.find((k) =>
        k.startsWith(features.charts.defaultType),
      );
  ...
  const currentOption = $derived(
    activeTab ? (specs[activeTab]?.option ?? {}) : {},
  );
  ...
  function handleBarClick(params: {
    ...
    const filterSeries =
      params.seriesName === "—" ? "Empty" : params.seriesName;
      ...
      .filter((issue) =>
        activeEntries.every(([key, vals]) => {
          ...
          if (raw == null || raw === "") v = "Empty";
            ...
            v = (raw as unknown[]).length ? fmtCell(key, raw) : "Empty";
        ...
        {#if tabKeys.length}
          <div class="cv-tabs-row">
              ...
              {#each tabKeys as key}
                ...
                >
                  {#if key === "explicit"}
                    ...
                    >
                      <path
                        d="M5 1v2M5 7v2M1 5h2M7 5h2M2.5 2.5l1.4 1.4M6.1 6.1l1.4 1.4M7.5 2.5L6.1 3.9M3.9 6.1L2.5 7.5"
                        stroke="currentColor"
                        stroke-width="1.2"
            ...
            {#if issues.length}
              ...
              <div class="cv-col-filter cv-axis-sel">
                <button
                  ...
                  class:active={!!axisX}
                  onclick={() => {
                    openAxisMenu = openAxisMenu === "x" ? null : "x";
                    axisSearch = "";
                  }}
                ...
                {#if openAxisMenu === "x"}
                  <div class="col-filter-menu cv-axis-menu">
                    ...
                    <div class="col-filter-list">
                      {#each allFields.filter((f) => !axisSearch || f
                            .toLowerCase()
                            .includes(axisSearch.toLowerCase())) as f}
                        <button
              ...
              <div class="cv-col-filter cv-axis-sel">
                <button
                  ...
                  onclick={() => {
                    openAxisMenu = openAxisMenu === "y" ? null : "y";
                    axisSearch = "";
                  }}
              ...
              <div class="cv-col-filter cv-axis-sel">
                <button
                  class="cv-react-btn"
                  onclick={() => {
                    openAxisMenu = openAxisMenu === "type" ? null : "type";
                  }}
            ...
            <button
              ...
              title={reactToFilters
                ? "Chart reacting to table filters — click to unlock"
                : "Chart showing all data — click to sync with filters"}
            >
              <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                <path
                  d="M1 2.5h8M2.5 5h5M4 7.5h2"
                  stroke="currentColor"
                  stroke-width="1.3"
                  stroke-linecap="round"
                />
            ...
            {#if chartStore.data?.jira_base_url && chartStore.data?.jql}
              ...
              >
                {hasActiveFilters || hasDateFilter
                  ...
                  : ""}{chartStore.data?.shown ?? issues.length}{chartStore
                  .data?.total
                  ? ` / ${chartStore.data.total}`
                  : ""}
              </a>
            {:else}
          ...
          {#if dateFields.length}
            <div class="cv-date-filter">
              <svg
                ...
                viewBox="0 0 10 10"
                fill="none"
                style="flex-shrink:0;color:#6b8aaa"
              >
                <rect
                  x="1"
                  y="1.5"
              ...
              <div class="cv-col-filter">
                <button
                  ...
                  onclick={() => {
                    openFilter =
                      openFilter === "__date_field__" ? null : "__date_field__";
                    filterSearch = "";
                  }}
                >
                  {dateField || "—"}
              ...
              <div class="cv-date-presets">
                ...
                <button
                  ...
                  onclick={() => setDatePreset("custom")}>Custom</button
                >
              </div>
              <!-- Custom date inputs -->
              {#if datePreset === "custom"}
              ...
              {:else if activeDateRange}
                <span class="cv-date-range-text">
                  {fmtDateShort(activeDateRange.from)} – {fmtDateShort(
                    activeDateRange.to,
                  )}
                </span>
              {/if}
              <!-- Count + clear -->
              {#if hasDateFilter}
                <span class="cv-date-count"
                  >{dateFilteredIssues.length} / {issues.length}</span
                >
                <button
                  class="cv-date-clear"
                ...
                <thead>
                  <tr>
                    ...
                    <th
                      ...
                      class="th-sortable"
                      class:th-sorted={sortCol === "summary"}
                    >
                      <div class="th-inner">
                    ...
                    {#each tableDisplayFields as col}
                      ...
                      <th
                        class:th-filtered={isFiltered}
                        class:th-sorted={sortCol === col}
                        class="th-sortable"
                            ...
                            <div class="cv-col-filter">
                              ...
                              {#if openFilter === col}
                                <div class="col-filter-menu">
                                  ...
                                  {/if}
                                  {#if colInfo}
                                    {@const visibleVals = colInfo.values.filter(
                                      (v) =>
                                        !filterSearch ||
                                        fmtDateKey(v)
                                          .toLowerCase()
                                      ...
                                      <button
                                        ...
                                        onclick={() =>
                                          allSelected
                                            ...
                                            : selectAllFilter(col, visibleVals)}
                                      >
                                        <span
                                          class="col-filter-check"
                                          class:checked={allSelected}
                                        >
                                          {#if allSelected}
                                            <svg
                                        ...
                                        <span class="col-filter-val"
                                          >{allSelected
                                            ? "Deselect all"
                                            : "Select all"}</span
                                        >
                                        <span class="col-filter-count"
                                          >{visibleVals.length}</span
                                        >
                                      </button>
                                    </div>
                                    <div class="col-filter-list">
  ...
  {:else}
    ...
    <div class="cv-empty">
      ...
      <p class="cv-empty-hint">
        Ask a question in the AI chat — charts and table will appear here
        automatically.
      </p>
    </div>
  {/if}
...
<style>
  ...
  .chart-view.dragging:has(.cv-split--h) {
    cursor: col-resize;
  }
  ...
  .cv-tab.ai.active {
    ...
    box-shadow:
      ...
      inset 0 0 8px rgba(129, 140, 248, 0.08);
  }
  .ai-spark {
    flex-shrink: 0;
  }
  ...
  .cv-divider:hover .cv-divider-dots {
    background: #818cf8;
  }
  ...
  .col-filter-item.empty-val .col-filter-val {
    color: #6b8aaa;
    font-style: italic;
  }
